...
import {convertSpeechToText} from '../utils/funcUtils';
...
const Speech = () => {
  ...
  console.log('DCM', results);
  const stopRecognizing = async () => {
    //Stops listening for speech
    try {
      await Voice.stop();
      setResults(['Bảng giá yêu thích']);
      //   navigation.navigate('MarkertOverview');
      convertSpeechToText({results, navigation});
    } catch (e) {
      //eslint-disable-next-line
      console.error(e);
    }
  ...
  const confirmShake = () => {
    Alert.alert('Alert Shake', "Let's say something !", [
      {
        text: 'Cancel',
        onPress: () => console.log('Cancel Pressed'),
        style: 'cancel'
      },
      {text: 'OK', onPress: () => startRecognizing()}
    ]);
  };
  useEffect(() => {
    //Setting callbacks for the process status
    const subscription = RNShake.addListener(() => confirmShake());
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.container}>
        ...
        <Text style={styles.textStyle}>
          Press mike to start Recognition or Shake
        </Text>
